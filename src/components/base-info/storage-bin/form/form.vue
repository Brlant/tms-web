...
<template>
  <dialog-template :pageSets="pageSets" @selectTab="selectTab">
    <template slot="title">{{title}}仓库信息</template>
    <template slot="btn">
      <el-button plain @click="onSubmit('form')" :disabled="doing">保存</el-button>
      <el-button plain @click="doClose" :disabled="doing">取消</el-button>
    </template>
          ...
          if (this.action === 'add') {
            ...
            GoodsArea.addGoodsAreaDetail(formData).then(() => {
              ...
              this.$notify.success({
                duration: 2000,
                name: '成功',
                message: '新增仓库"' + self.form.name + '"单位成功'
              });
              self.$emit('change', formData);
            }).catch(error => {
              this.$notify.error({
                duration: 2000,
                message: error.response && error.response.data && error.response.data.msg || '新增仓库单位失败'
              });
